				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr>
							...
							<td><a href="{{ asset('/upload/cartFile/'.$product->custom_design_file_upload) }}" download>{{ $product->custom_design_file_upload }}</a></td>
							<td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $product->created_at)->format('m/d/Y') }}</td>
						</tr>
					</tbody>
				</table>
				<table class="table table-striped table-bordered">
								...
								<td>
									@if(!empty($productdetail->file))
										@foreach(json_decode($productdetail->file) as $value)
											<a href="{{ route('doc.download', ['file' => urlencode($value)]) }}" class="btn btn-primary download-btn" download="{{$value}}">{{ $value }}<i class="fa fa-download download-icon"></i></a>
										@endforeach
									@endif
								</td>
								<td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $productdetail->created_at)->format('m/d/Y')  }}</td>
							</tr>
							@endforeach
						@else
							<tr>
