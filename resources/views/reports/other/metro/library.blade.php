<div class="row">
    <div class="col-lg-5 col-md-5 col-sm-5">
        <div class="left-side colora">
            {{__('reports/other/metro/library.publicLibrary')}}
        </div>
    </div>
    <div class="col-lg-7 col-md-7 col-sm-7 school-item-container">
        <div class="row" id="librarydata">
            <div class="col-lg-6 col-md-6 col-sm-6">
                <div class="row">
                    <div class="col-lg-3 col-md-3 col-sm-2 public-library-text">
                        <div class="row">
                            <div class="icon backgrounde">
                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000 1000" enable-background="new 0 0 1000 1000" xml:space="preserve" class="fillf">
                                <metadata> Svg Vector Icons : http://www.onlinewebfonts.com/icon </metadata>
                                <g><g transform="translate(0.000000,511.000000) scale(0.100000,-0.100000)"><path d="M333.8,4801c-37.9-11.1-91.3-42.3-120.3-69C120,4645.1,100,4556,100,4244.2v-289.5h1002.1h1002.2v289.5c0,343-28.9,440.9-153.7,516.7c-77.9,51.2-93.5,51.2-815.1,55.7C732.5,4818.8,369.5,4812.1,333.8,4801z"/><path d="M2839.2,4201.9c-138.1-73.5-167-151.4-173.7-496.6l-8.9-307.3h714.9h714.9v314c0,278.4-4.5,325.1-44.5,387.5c-82.4,131.4-140.3,144.8-661.4,144.8C2970.6,4244.2,2908.3,4239.8,2839.2,4201.9z"/><path d="M7177.4,3687.5c-376.3-84.6-706-167-732.7-187.1c-77.9-53.4-131.4-151.4-131.4-238.3c0-102.4,46.8-331.8,66.8-331.8c31.2,0,1917.4,418.7,1926.4,427.6c4.5,4.4-6.7,77.9-26.7,160.3c-24.5,111.3-53.4,173.7-104.7,231.6C8059.3,3874.5,7981.4,3870.1,7177.4,3687.5z"/><path d="M100-31.6v-3140.1h1002.1h1002.2V-31.6v3140.1H1102.1H100V-31.6z"/><path d="M2665.5-169.7l6.7-2990.9l708.2-6.7l706-4.5v2995.3v2995.3h-712.6H2661L2665.5-169.7z"/><path d="M7449.1,2580.6c-521.1-115.8-948.7-213.8-950.9-216c-8.9-8.9,1267.2-5814.7,1282.8-5828.1c6.7-6.7,447.6,82.4,977.6,200.4c792.8,173.7,964.3,218.2,955.4,247.2c-4.5,17.8-291.7,1334-641.4,2921.8c-347.4,1590.1-641.4,2888.4-654.7,2888.4C8406.7,2792.2,7970.2,2696.4,7449.1,2580.6z"/><path d="M4865.8,2513.8c-17.8-6.7-57.9-33.4-84.6-60.1c-102.4-91.3-115.8-149.2-115.8-469.9v-300.6h857.4h857.4v318.5c0,347.4-15.6,398.6-147,485.5c-60.1,37.9-113.6,42.3-699.3,40.1C5184.2,2527.2,4883.6,2520.5,4865.8,2513.8z"/><path d="M4665.4-1033.8v-2137.9h857.4h857.4v2137.9v2137.9h-857.4h-857.4V-1033.8z"/><path d="M8836.5-3826.4c-503.3-111.3-922-207.1-926.4-213.8c-17.8-17.8,51.2-296.2,86.8-351.9c46.8-71.3,162.6-138.1,238.3-138.1c73.5,0,1376.3,280.6,1469.8,316.2c33.4,13.3,91.3,57.9,126.9,100.2c75.7,84.6,86.9,187.1,40.1,383c-20,93.5-33.4,113.6-71.3,111.3C9774.1-3621.6,9342.1-3712.9,8836.5-3826.4z"/><path d="M115.6-3744c-8.9-6.7-15.6-140.3-15.6-294c0-354.1,35.6-447.6,189.3-516.7c95.8-42.3,140.3-44.5,852.9-37.9c734.9,6.7,752.7,6.7,819.5,55.7c120.3,86.8,142.5,164.8,142.5,505.5v302.9h-986.6C574.3-3728.4,122.3-3735.1,115.6-3744z"/><path d="M2670-3761.9c-6.7-17.8-8.9-160.4-4.4-316.2c13.4-485.5,57.9-518.9,714.9-518.9c487.7,0,543.4,11.1,645.8,131.4c60.1,71.2,60.1,75.7,60.1,405.3v331.8h-701.5C2783.5-3728.4,2681.1-3732.9,2670-3761.9z"/><path d="M4669.8-4060.3c6.7-296.2,11.1-340.7,53.4-396.4c98-133.6,138.1-140.3,795-140.3c645.8,0,688.2,6.7,795,120.3c53.5,60.1,55.7,73.5,62.3,405.3l6.7,343h-859.6h-859.6L4669.8-4060.3z"/></g></g>
                                </svg>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-9 col-md-9 col-sm-9 public-library-text">
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <div class="row backgroundd colorb heading1">
                                    {{__('reports/other/metro/library.closestBranch')}}
                                </div>
                            </div>
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <div class="row backgroundd colorb heading2">
                                    {{__('reports/other/metro/library.freshViewBranch')}}
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-6" id="libraryvalue">
                <div class="row">
                    <div class="library-address">
                        <div id="lib_name">
                            {{ (!empty($response) && !empty($response->name)) ? $response->name : 'N/A' }}
                        </div>
                        <div id="lib_address">
                            {{ (!empty($response) && !empty($response->address)) ? $response->address : 'N/A' }}
                        </div>
                        <div class="distance-container">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 470.642 470.642" style="enable-background:new 0 0 470.642 470.642;" xml:space="preserve" class="filla walk-man">
                                <g> <g> <path style="fill:#010002;" d="M223.821,76.022c10.333-0.333,19-4.333,26-12s10.333-16.667,10-27    c-0.333-10.335-4.167-19.168-11.5-26.5c-7.333-7.333-16.167-10.833-26.5-10.5s-19,4.333-26,12s-10.5,16.667-10.5,27    s3.833,19.167,11.5,26.5S213.488,76.355,223.821,76.022z"></path> <path style="fill:#010002;" d="M350.321,202.522l-55-30l-45-72c-8.667-10.667-19.333-16-32-16c-8.667,0-17,3.667-25,11l-68,69    c-2,2.667-3.333,5.667-4,9l-9,77v2c0,4.666,1.667,8.666,5,12c3.333,3.332,7.333,5,12,5s8.5-1.668,11.5-5c3-3.334,4.833-7,5.5-11    l7-66l24-24l-22,184l-39,87.001c-1.333,4-2,7.658-2,11.002c0,7.322,2.5,13.5,7.5,18.5s11.167,7.164,18.5,6.5    c10,0,17.333-4.678,22-14l42-94.002c0-0.67,0.333-1.835,1-3.5c0.668-1.67,1.335-3.17,2-4.5c0.667-1.335,1-2.67,1-4l5-45.001    l43,148.003c4.667,12,13,17.666,25,17c6.667,0,12.5-2.5,17.5-7.5s7.5-11.178,7.5-18.5c0-0.678-0.167-1.5-0.5-2.5    s-0.5-1.836-0.5-2.502l-60-205.001l7-67l17,27c1.333,2,3,3.667,5,5l59,33c4,1.333,6.667,2,8,2c4.667,0,8.667-1.833,12-5.5    c3.333-3.668,5-7.834,5-12.5C358.321,210.522,355.654,205.855,350.321,202.522z"></path> </g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g>
                            </svg>
                            <span class="walk-man-text2" style="display:inline;"><strong class="color1">{{__('reports/other/metro/library.distance')}}</strong>:</span>
                            <span class="walk-man-text2" style="display:inline;" id="lib_distance">
                                {{ (!empty($response) && !empty($response->distance)) ? round($response->distance, 2).'KM'  : 'N/A' }}
                            </span>
                        </div>  
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="row">
    <hr class="style4">
</div>