<div class="row">
    <div class="col-lg-5 col-md-5 col-sm-5">
        <div class="left-side colora">
            WASTE COLLECTION SCHEDULE
        </div>
    </div>
    <div class="col-lg-7 col-md-7 col-sm-7 waste-item-container">
        <div class="row">
            <div class="col-lg-4 col-md-4 col-sm-4">
                <div class="row item waste1">
                    <div class="waste-head main-icon backgrounde">
                        <svg version="1.1" id="Capa_1" class="fillf" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                            viewBox="0 0 99.776 99.777" style="enable-background:new 0 0 99.776 99.777;"
                            xml:space="preserve">
                            <g>
                            <g>
                            <path d="M78.104,15.759H21.673c-3.703,0-6.708,0.421-6.708,3.424l4.019,75.166c0,2.997,3.001,5.428,6.704,5.428h47.053
                            c3.706,0,6.704-2.431,6.704-5.428l5.359-75.166C84.812,16.18,81.811,15.759,78.104,15.759z M67.749,58.341l2.686,6.354
                            c0,0.346-0.283,0.64-0.633,0.64h-6.051c-0.352,0-1.026-0.314-1.3-0.64l-1.649-3.378c-0.298-0.52,0.027-0.722,0.499-0.924
                            l5.441-2.519C67.522,57.482,67.749,58.341,67.749,58.341z M47.955,38.433c0.674-0.382,7.187-1.624,7.665-1.893
                            c0.476-0.281,0.678,0.091,0.678,0.091c0.896,1.581,3.97,6.935,5.182,9.062c0.396,0.688,0.65,1.136,0.65,1.136
                            s1.648-0.942,2.683-1.538c1.026-0.585,0.698,0.633,0.698,0.633s-2.307,9.043-2.563,10.005c-0.188,0.695-0.894,0.441-0.894,0.441
                            l-9.974-2.56c0,0-1.21-0.294-0.361-0.773c0.853-0.496,2.905-1.667,2.905-1.667s-0.287-0.503-0.722-1.27
                            c-1.258-2.193-5.323-9.282-6.252-10.911C47.654,39.192,47.282,38.817,47.955,38.433z M45.517,73.932
                            c0.027,0.876-0.852,0.712-0.852,0.712l-6.893-0.28c-0.316-0.15-0.448-0.523-0.297-0.849l2.578-5.473
                            c0.147-0.314,0.715-0.787,1.125-0.903l3.754-0.051c0.603-0.059,0.645,0.338,0.626,0.842L45.517,73.932z M45.138,62.091
                            c-0.886-0.41-3.025-1.431-3.025-1.431s-0.252,0.524-0.623,1.314c-1.08,2.289-4.57,9.685-5.369,11.383c0,0-0.094,0.51-0.792,0.181
                            c-0.703-0.332-5.452-4.966-5.946-5.194c-0.501-0.229-0.305-0.616-0.305-0.616c0.773-1.643,3.412-7.225,4.454-9.435
                            c0.335-0.712,0.556-1.188,0.556-1.188s-1.716-0.812-2.799-1.318c-1.073-0.506,0.123-0.931,0.123-0.931s8.731-3.285,9.671-3.627
                            c0.676-0.24,0.876,0.479,0.876,0.479l3.614,9.637C45.573,61.345,46.024,62.505,45.138,62.091z M46.694,50.036l-4.892-3.474
                            c-0.739-0.482-0.094-1.104-0.094-1.104l4.17-5.484c0.305-0.171,0.693-0.066,0.866,0.231l3.003,5.256
                            c0.178,0.305,0.245,1.037,0.098,1.444l-2.103,3.105C47.437,50.525,47.092,50.347,46.694,50.036z M68.577,74.856
                            c0,0.548-0.432,0.541-0.432,0.541c-1.813,0-7.987,0-10.431,0c-0.787,0-1.308,0-1.308,0s0,1.896,0,3.087
                            c0,1.194-0.89,0.287-0.89,0.287s-6.707-6.491-7.412-7.193c-0.512-0.506,0.06-0.999,0.06-0.999l7.182-7.378
                            c0,0,0.849-0.903,0.849,0.075c0,0.982,0,3.347,0,3.347s0.582,0,1.458,0c2.529,0,10.711,0,12.58,0c0,0,0.506-0.13,0.506,0.64
                            C70.736,68.033,68.577,74.298,68.577,74.856z"/>
                            <path d="M19.269,13.362h61.238c2.375,0,4.305-1.927,4.305-4.303c0-2.375-1.924-4.307-4.305-4.307H64.412
                            C64.412,2.127,62.287,0,59.665,0H40.116c-2.625,0-4.751,2.127-4.751,4.751H19.272c-2.375,0-4.303,1.929-4.303,4.307
                            C14.965,11.435,16.894,13.362,19.269,13.362z"/>
                            </g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                        </svg>
                        <span class="waste-title colorb">
                            Blue Cart <span class="waste-sub-title">(Recycling)</span>
                        </span>
                    </div>
                    <div class="info">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-1">
                                    <div class="icon">
                                        <svg version="1.1" id="Capa_1" class="filla" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                                            <g>
                                            <path d="M160,256H96v-64h64V256z M288,192h-64v64h64V192z M416,192h-64v64h64V192z M160,288H96v64h64V288z M288,288h-64v64h64V288z
                                            M416,288h-64v64h64V288z M160,384H96v64h64V384z M288,384h-64v64h64V384z M128,96c17.672,0,32-14.313,32-32V32
                                            c0-17.688-14.328-32-32-32S96,14.313,96,32v32C96,81.688,110.328,96,128,96z M512,64v448H0V64h80c0,26.469,21.531,48,48,48
                                            c26.469,0,48-21.531,48-48h160c0,26.469,21.531,48,48,48s48-21.531,48-48H512z M480,160H32v320h448V160z M384,96
                                            c17.688,0,32-14.313,32-32V32c0-17.688-14.312-32-32-32s-32,14.313-32,32v32C352,81.688,366.312,96,384,96z"/>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                        </svg>
                                    </div>
                                </div>
                                <div class="col-lg-10 col-md-10 col-sm-9">
                                    <div class="waste-collection color-black">
                                        <div class="colora">
                                        Collection day: 
                                        </div>
                                        @if (isset($waste_blue_cart) && isset($waste_blue_cart[0]) && isset($waste_blue_cart[0]->collection_day))
                                            {{$waste_blue_cart[0]->collection_day}}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-1">
                                    <div class="icon">
                                        <svg version="1.1" id="Capa_1" class="filla" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                             viewBox="0 0 485.213 485.212" style="enable-background:new 0 0 485.213 485.212;"
                                             xml:space="preserve">
                                        <g>
                                            <path d="M60.652,75.816V15.163C60.652,6.781,67.433,0,75.817,0c8.38,0,15.161,6.781,15.161,15.163v60.653
                                                c0,8.38-6.781,15.161-15.161,15.161C67.433,90.978,60.652,84.196,60.652,75.816z M318.424,90.978
                                                c8.378,0,15.163-6.781,15.163-15.161V15.163C333.587,6.781,326.802,0,318.424,0c-8.382,0-15.168,6.781-15.168,15.163v60.653
                                                C303.256,84.196,310.042,90.978,318.424,90.978z M485.212,363.906c0,66.996-54.312,121.307-121.303,121.307
                                                c-66.986,0-121.302-54.311-121.302-121.307c0-66.986,54.315-121.3,121.302-121.3C430.9,242.606,485.212,296.919,485.212,363.906z
                                                 M454.89,363.906c0-50.161-40.81-90.976-90.98-90.976c-50.166,0-90.976,40.814-90.976,90.976c0,50.171,40.81,90.98,90.976,90.98
                                                C414.08,454.886,454.89,414.077,454.89,363.906z M121.305,181.955H60.652v60.651h60.653V181.955z M60.652,333.584h60.653V272.93
                                                H60.652V333.584z M151.629,242.606h60.654v-60.651h-60.654V242.606z M151.629,333.584h60.654V272.93h-60.654V333.584z
                                                 M30.328,360.891V151.628h333.582v60.653h30.327V94c0-18.421-14.692-33.349-32.843-33.349h-12.647v15.166
                                                c0,16.701-13.596,30.325-30.322,30.325c-16.731,0-30.326-13.624-30.326-30.325V60.651H106.14v15.166
                                                c0,16.701-13.593,30.325-30.322,30.325c-16.733,0-30.327-13.624-30.327-30.325V60.651H32.859C14.707,60.651,0.001,75.579,0.001,94
                                                v266.892c0,18.36,14.706,33.346,32.858,33.346h179.424v-30.331H32.859C31.485,363.906,30.328,362.487,30.328,360.891z
                                                 M303.256,242.606v-60.651h-60.648v60.651H303.256z M409.399,363.906h-45.49v-45.49c0-8.377-6.781-15.158-15.163-15.158
                                                s-15.159,6.781-15.159,15.158v60.658c0,8.378,6.777,15.163,15.159,15.163h60.653c8.382,0,15.163-6.785,15.163-15.163
                                                C424.562,370.692,417.781,363.906,409.399,363.906z"/>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        </svg>
                                    </div>
                                </div>
                                <div class="col-lg-10 col-md-10 col-sm-9">
                                    <div class="waste-collection color-black">
                                        <div class="colora">
                                        Frequency: 
                                        </div>
                                        @if (isset($waste_blue_cart) && isset($waste_blue_cart[0]) && isset($waste_blue_cart[0]->clect_int_summer))
                                            {{$waste_blue_cart[0]->clect_int_summer}}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-4 col-sm-4">
                <div class="row item waste2">
                    <div class="waste-head main-icon backgrounde">
                        <svg version="1.1" id="Capa_1" class="fillf" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                 viewBox="0 0 97.463 97.463" style="enable-background:new 0 0 97.463 97.463;" xml:space="preserve">
                            <g>
                                <g>
                                    <path d="M59.638,75.934V34.451h6.383v-0.508l0.096,0.08l24.365-19.946l-5.947-4.935L69.846,21.173l1.653-5.534L17.029,0
                                        l-5.291,9.433l46.33,13.846H6.981l6.232,11.171h4.454L29.729,96.06l20.892-0.37c-1.822-2.098-2.953-4.797-2.953-7.788
                                        C47.671,81.296,53.03,75.934,59.638,75.934z"/>
                                    <path d="M60.696,78.347c-5.271,0-9.557,4.291-9.557,9.558s4.286,9.559,9.557,9.559c5.264,0,9.555-4.292,9.555-9.559
                                        S65.959,78.347,60.696,78.347z M60.696,93.432c-3.049,0-5.53-2.481-5.53-5.53c0-3.05,2.481-5.527,5.53-5.527
                                        c3.046,0,5.526,2.478,5.526,5.527C66.222,90.951,63.745,93.432,60.696,93.432z"/>
                                </g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            <g>
                            </g>
                            </svg>
                        <span class="waste-title colorb">
                            Black Cart <span class="waste-sub-title">(Garbage)</span>
                        </span>
                    </div>
                    <div class="info">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-1">
                                    <div class="icon">
                                        <svg version="1.1" id="Capa_1" class="filla" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                                            <g>
                                            <path d="M160,256H96v-64h64V256z M288,192h-64v64h64V192z M416,192h-64v64h64V192z M160,288H96v64h64V288z M288,288h-64v64h64V288z
                                            M416,288h-64v64h64V288z M160,384H96v64h64V384z M288,384h-64v64h64V384z M128,96c17.672,0,32-14.313,32-32V32
                                            c0-17.688-14.328-32-32-32S96,14.313,96,32v32C96,81.688,110.328,96,128,96z M512,64v448H0V64h80c0,26.469,21.531,48,48,48
                                            c26.469,0,48-21.531,48-48h160c0,26.469,21.531,48,48,48s48-21.531,48-48H512z M480,160H32v320h448V160z M384,96
                                            c17.688,0,32-14.313,32-32V32c0-17.688-14.312-32-32-32s-32,14.313-32,32v32C352,81.688,366.312,96,384,96z"/>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                        </svg>
                                    </div>
                                </div>
                                <div class="col-lg-10 col-md-10 col-sm-9">
                                    <div class="waste-collection color-black">
                                        <div class="colora">
                                        Collection day: 
                                        </div>
                                        @if (isset($waste_black_cart) && isset($waste_black_cart[0]) && isset($waste_black_cart[0]->collection_day))
                                            {{$waste_black_cart[0]->collection_day}}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-1">
                                    <div class="icon">
                                        <svg version="1.1" id="Capa_1" class="filla" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                             viewBox="0 0 485.213 485.212" style="enable-background:new 0 0 485.213 485.212;"
                                             xml:space="preserve">
                                        <g>
                                            <path d="M60.652,75.816V15.163C60.652,6.781,67.433,0,75.817,0c8.38,0,15.161,6.781,15.161,15.163v60.653
                                                c0,8.38-6.781,15.161-15.161,15.161C67.433,90.978,60.652,84.196,60.652,75.816z M318.424,90.978
                                                c8.378,0,15.163-6.781,15.163-15.161V15.163C333.587,6.781,326.802,0,318.424,0c-8.382,0-15.168,6.781-15.168,15.163v60.653
                                                C303.256,84.196,310.042,90.978,318.424,90.978z M485.212,363.906c0,66.996-54.312,121.307-121.303,121.307
                                                c-66.986,0-121.302-54.311-121.302-121.307c0-66.986,54.315-121.3,121.302-121.3C430.9,242.606,485.212,296.919,485.212,363.906z
                                                 M454.89,363.906c0-50.161-40.81-90.976-90.98-90.976c-50.166,0-90.976,40.814-90.976,90.976c0,50.171,40.81,90.98,90.976,90.98
                                                C414.08,454.886,454.89,414.077,454.89,363.906z M121.305,181.955H60.652v60.651h60.653V181.955z M60.652,333.584h60.653V272.93
                                                H60.652V333.584z M151.629,242.606h60.654v-60.651h-60.654V242.606z M151.629,333.584h60.654V272.93h-60.654V333.584z
                                                 M30.328,360.891V151.628h333.582v60.653h30.327V94c0-18.421-14.692-33.349-32.843-33.349h-12.647v15.166
                                                c0,16.701-13.596,30.325-30.322,30.325c-16.731,0-30.326-13.624-30.326-30.325V60.651H106.14v15.166
                                                c0,16.701-13.593,30.325-30.322,30.325c-16.733,0-30.327-13.624-30.327-30.325V60.651H32.859C14.707,60.651,0.001,75.579,0.001,94
                                                v266.892c0,18.36,14.706,33.346,32.858,33.346h179.424v-30.331H32.859C31.485,363.906,30.328,362.487,30.328,360.891z
                                                 M303.256,242.606v-60.651h-60.648v60.651H303.256z M409.399,363.906h-45.49v-45.49c0-8.377-6.781-15.158-15.163-15.158
                                                s-15.159,6.781-15.159,15.158v60.658c0,8.378,6.777,15.163,15.159,15.163h60.653c8.382,0,15.163-6.785,15.163-15.163
                                                C424.562,370.692,417.781,363.906,409.399,363.906z"/>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        </svg>
                                    </div>
                                </div>
                                <div class="col-lg-10 col-md-10 col-sm-9">
                                    <div class="waste-collection color-black">
                                        <div class="colora">
                                        Frequency: 
                                        </div>
                                        @if (isset($waste_black_cart) && isset($waste_black_cart[0]) && isset($waste_black_cart[0]->clect_int_summer))
                                            {{$waste_black_cart[0]->clect_int_summer}}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-4 col-sm-4">
                <div class="row item waste2">
                    <div class="waste-head main-icon backgrounde">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve" class="fillf">
                                <path fill="#000000" d="M86.114,24.883c-0.488-0.554-1.193-0.872-1.932-0.872H15.958c-0.74,0-1.443,0.318-1.932,0.872  c-0.489,0.555-0.717,1.292-0.624,2.026l8.087,64.383c0.162,1.288,1.258,2.255,2.557,2.255h52.046c1.299,0,2.396-0.967,2.558-2.255  l8.089-64.383C86.831,26.175,86.604,25.438,86.114,24.883z M58.843,52.007c-2.185,0.689-3.771,2.732-3.771,5.146  c0,1.218,0.409,2.338,1.09,3.241c-0.681,0.903-1.09,2.022-1.09,3.241c0,2.414,1.587,4.455,3.771,5.146  c-0.109,0.427-0.173,0.873-0.173,1.335c0,1.4,0.538,2.671,1.412,3.631c-2.081,1.081-4.441,1.696-6.949,1.696  c-1.05,0-2.072-0.106-3.062-0.312c-0.99,0.205-2.013,0.312-3.062,0.312c-2.509,0-4.869-0.615-6.95-1.696  c0.875-0.96,1.413-2.23,1.413-3.631c0-0.462-0.064-0.908-0.173-1.335c2.185-0.69,3.771-2.731,3.771-5.146  c0-1.219-0.41-2.338-1.09-3.241c0.68-0.903,1.09-2.023,1.09-3.241c0-2.413-1.586-4.456-3.771-5.146  c0.109-0.428,0.173-0.874,0.173-1.335c0-1.423-0.555-2.714-1.456-3.678c2.09-1.096,4.467-1.719,6.993-1.719  c0.459,0,0.913,0.024,1.362,0.065c0.069-0.566,0.858-6.251,4.637-8.631c0.805-0.507,1.864-0.265,2.369,0.538  c0.506,0.803,0.265,1.863-0.538,2.369c-1.863,1.173-2.729,4.197-2.994,5.719c0.425-0.036,0.854-0.059,1.288-0.059  c2.524,0,4.901,0.623,6.992,1.719c-0.9,0.964-1.455,2.255-1.455,3.678C58.67,51.133,58.733,51.579,58.843,52.007z"></path><path fill="#000000" d="M85.189,12.147H60.02c0.002-0.046,0.007-0.091,0.007-0.138c0-2.372-1.923-4.294-4.295-4.294H44.46  c-2.372,0-4.294,1.922-4.294,4.294c0,0.047,0.005,0.092,0.007,0.138h-25.17c-2.372,0-4.294,1.922-4.294,4.294  c0,2.372,1.922,4.294,4.294,4.294h70.187c2.372,0,4.295-1.922,4.295-4.294C89.484,14.07,87.562,12.147,85.189,12.147z"></path>
                            </svg>
                        <span class="waste-title colorb">
                            Green Cart <span class="waste-sub-title">(Compost)</span>
                        </span>
                    </div>
                    <div class="info">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-1">
                                    <div class="icon">
                                        <svg version="1.1" id="Capa_1" class="filla" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                                            <g>
                                            <path d="M160,256H96v-64h64V256z M288,192h-64v64h64V192z M416,192h-64v64h64V192z M160,288H96v64h64V288z M288,288h-64v64h64V288z
                                            M416,288h-64v64h64V288z M160,384H96v64h64V384z M288,384h-64v64h64V384z M128,96c17.672,0,32-14.313,32-32V32
                                            c0-17.688-14.328-32-32-32S96,14.313,96,32v32C96,81.688,110.328,96,128,96z M512,64v448H0V64h80c0,26.469,21.531,48,48,48
                                            c26.469,0,48-21.531,48-48h160c0,26.469,21.531,48,48,48s48-21.531,48-48H512z M480,160H32v320h448V160z M384,96
                                            c17.688,0,32-14.313,32-32V32c0-17.688-14.312-32-32-32s-32,14.313-32,32v32C352,81.688,366.312,96,384,96z"/>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                            <g>
                                            </g>
                                        </svg>
                                    </div>
                                </div>
                                <div class="col-lg-10 col-md-10 col-sm-9">
                                    <div class="waste-collection color-black">
                                        <div class="colora">
                                        Collection day: 
                                        </div>
                                        @if (isset($waste_green_cart) && isset($waste_green_cart[0]) && isset($waste_green_cart[0]->collection_day))
                                            {{$waste_green_cart[0]->collection_day}}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-1">
                                    <div class="icon">
                                        <svg version="1.1" id="Capa_1" class="filla" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                             viewBox="0 0 485.213 485.212" style="enable-background:new 0 0 485.213 485.212;"
                                             xml:space="preserve">
                                        <g>
                                            <path d="M60.652,75.816V15.163C60.652,6.781,67.433,0,75.817,0c8.38,0,15.161,6.781,15.161,15.163v60.653
                                                c0,8.38-6.781,15.161-15.161,15.161C67.433,90.978,60.652,84.196,60.652,75.816z M318.424,90.978
                                                c8.378,0,15.163-6.781,15.163-15.161V15.163C333.587,6.781,326.802,0,318.424,0c-8.382,0-15.168,6.781-15.168,15.163v60.653
                                                C303.256,84.196,310.042,90.978,318.424,90.978z M485.212,363.906c0,66.996-54.312,121.307-121.303,121.307
                                                c-66.986,0-121.302-54.311-121.302-121.307c0-66.986,54.315-121.3,121.302-121.3C430.9,242.606,485.212,296.919,485.212,363.906z
                                                 M454.89,363.906c0-50.161-40.81-90.976-90.98-90.976c-50.166,0-90.976,40.814-90.976,90.976c0,50.171,40.81,90.98,90.976,90.98
                                                C414.08,454.886,454.89,414.077,454.89,363.906z M121.305,181.955H60.652v60.651h60.653V181.955z M60.652,333.584h60.653V272.93
                                                H60.652V333.584z M151.629,242.606h60.654v-60.651h-60.654V242.606z M151.629,333.584h60.654V272.93h-60.654V333.584z
                                                 M30.328,360.891V151.628h333.582v60.653h30.327V94c0-18.421-14.692-33.349-32.843-33.349h-12.647v15.166
                                                c0,16.701-13.596,30.325-30.322,30.325c-16.731,0-30.326-13.624-30.326-30.325V60.651H106.14v15.166
                                                c0,16.701-13.593,30.325-30.322,30.325c-16.733,0-30.327-13.624-30.327-30.325V60.651H32.859C14.707,60.651,0.001,75.579,0.001,94
                                                v266.892c0,18.36,14.706,33.346,32.858,33.346h179.424v-30.331H32.859C31.485,363.906,30.328,362.487,30.328,360.891z
                                                 M303.256,242.606v-60.651h-60.648v60.651H303.256z M409.399,363.906h-45.49v-45.49c0-8.377-6.781-15.158-15.163-15.158
                                                s-15.159,6.781-15.159,15.158v60.658c0,8.378,6.777,15.163,15.159,15.163h60.653c8.382,0,15.163-6.785,15.163-15.163
                                                C424.562,370.692,417.781,363.906,409.399,363.906z"/>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        <g>
                                        </g>
                                        </svg>
                                    </div>
                                </div>
                                <div class="col-lg-10 col-md-10 col-sm-9">
                                    <div class="waste-collection color-black">
                                        <div class="colora">
                                        Frequency: 
                                        </div>
                                        @if (isset($waste_green_cart) && isset($waste_green_cart[0]) && isset($waste_green_cart[0]->clect_int_summer))
                                            {{$waste_green_cart[0]->clect_int_summer}}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="row">
    <hr class="style4">
</div>