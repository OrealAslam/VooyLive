			...
			<tr>
				...
				<td>
					<div class="icon-container" style="">
						@if(Storage::exists('/hdi_icons/'.$icon->icon_file))
							{!!Storage::get('/hdi_icons/'.$icon->icon_file)!!}
						@else
							N/A
						@endif
					</div>
				...
				<td>
					@foreach ($icon->IconTags as $iconTag)
						<a href="{{URL::Route('upsertHdiIconTags', ['id' => $iconTag->Tag->id])}}" class="btn btn-info btn-xs">{{$iconTag->Tag->name}}</a>
					@endforeach
				</td>
